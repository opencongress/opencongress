...
#               :facets => [:bill_type, :session], :auto_commit => false
  ...
  has_many :actions, :order => 'actions.date DESC'
  ...
  has_many :amendments, :order => 'retreived_date', :include => :roll_calls
...
#    Bill.find_by_session_and_bill_type_and_number(session, bill_type, number, find_options)
  ...
  def top_recipients_for_all_interest_groups(disposition = 'support', chamber = 'house', num = 10)
    ...
    WHERE crp_interest_group_osid IN (?) AND cycle='2008'
     GROUP BY recipient_osid)
     top_recips ON people.osid=top_recips.recipient_osid
     WHERE people.title=?
     ORDER BY top_recips.contrib_total DESC LIMIT ?", groups_ids, title, num])
  end
  
  def Bill.client_id_to_url(client_id)
    client_id.slice!(/\d+_/)
    long_type_to_short(client_id)
  end

  def Bill.from_param(param)
  ...
  def Bill.canonical_name(name)
    "#{name.gsub(/[\.\s\/]+/,"").downcase}"
  end
  
  def Bill.ident(param_id)

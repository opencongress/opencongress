...
Facebox.prototype = {	
	...
	initialize	: function(extra_set){
	  ...
	  <div id="facebox" style="display:none;"> \
		...
		new Insertion.Bottom($$('body').first(), this.settings.facebox_html);
//		$$('body').first().insert({bottom: this.settings.facebox_html});
	...
	loading	: function() {
		...
		new Insertion.Bottom(contentWrapper, '<div class="loading"><img src="'+this.settings.loading_image+'"/></div>');
...
//		Event.observe(document, 'keypress', this.keyPressListener);
	...
	reveal	: function(data, klass){
		...
		new Insertion.Bottom(contentWrapper,data);
...
//		Event.observe(document, 'keypress', this.keyPressListener);
	...
	click_handler	: function(elem, e){
		...
		if (elem.href.match(/#/)){
			...
			var data = "<" + d.tagName + ">" + d.innerHTML + "</" + d.tagname + ">";

  ...
  klass.module_eval do
    ...
    def include_javascripts(*packages)
      tags = packages.map do |pack|
        Jammit.package_assets ? Jammit.asset_url(pack, (defined?(request.env['HTTP_ACCEPT_ENCODING']) && request.env['HTTP_ACCEPT_ENCODING'].scan('gzip').length > 0 ? 'js.gz' : :js)) : Jammit.packager.individual_urls(pack.to_sym, :js)
      end
    ...
    def packaged_stylesheets(packages, options)
      tags_with_options(packages, options) {|p| Jammit.asset_url(p, (defined?(request.env['HTTP_ACCEPT_ENCODING']) && request.env['HTTP_ACCEPT_ENCODING'].scan('gzip').length > 0 ? 'css.gz' : :css)) }
    end

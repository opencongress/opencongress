...
module S3sync
	...
	#defined below out of necessity of the 'require' order 
   #$AWS_CALLING_FORMAT
	...
	$AWS_CALLING_FORMAT = S3::CallingFormat::string_to_format((ENV["AWS_CALLING_FORMAT"] or "REGULAR"))
	...
	def S3sync.s3trySetup 	
      ...
      $S3syncConnection.calling_format = $AWS_CALLING_FORMAT
		...
		while $S3syncRetriesLeft > 0 do
         ...
         hush = false
            ...
            if result.http_response.code.to_i == 301
               ...
               debug("Host: " + host)
               $S3syncHttp = $S3syncConnection.make_http(bucket, host)
